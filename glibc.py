...
LIBC_REPOSITORY="git://sourceware.org/git/glibc.git"

BACKSLASHLFRE = re.compile(r"\\\n")
PREPROCRE = re.compile(r"^\s*#.*$", re.M)
FUNCREFMT = r"extern\s[^;{{]+(?:\s|\*){}\s*\(.*?\)\s*;"

def _get():
    root = cache.get_from_git(LIBC_REPOSITORY, "master")
    cache.configure(root)
    return cache.make(root, "install-headers")
